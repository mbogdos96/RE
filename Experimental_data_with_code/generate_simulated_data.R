...
generate_data_frCN <- function(n, 
  ...
  range1 <- runif(n_half + remainder, 
                  3.6, 3.8)
  range2 <- runif(n_half, 
                  4.2, 4.4)
  
  L_type <- as.numeric(sample(c(range1, 
                                range2)))
  ...
  if (ground_truth_model == 1) {
    ...
    logk <- intercept + 
      coef_E_ox * data$E_ox_scaled + 
      coef_V_bur * data$V_bur_scaled + 
      coef_L_type * data$L_type_scaled + 
      epsilon
    ground_truth <- c(intercept = intercept, 
                      coef_E_ox = coef_E_ox, 
                      coef_V_bur = coef_V_bur, 
...
# Function for fitting models for fractional CN
fit_models_frCN <- function(input) {
  ...
  model1 <- lm(logk ~ E_ox_scaled, 
               data = data)
  model2 <- lm(logk ~ E_ox_scaled 
               + V_bur_scaled,
               data = data)
  model3 <- lm(logk ~ E_ox_scaled 
               + V_bur_scaled 
               + L_type_scaled, 
               data = data)
...
# Run simulations
#================
run_simulations <- function(n_obs_vec, 
                            noise_sd_vec, 
                            n_simulations,
                            cn_is) {
      ...
      for (ground_truth_model in 1:3) {
        for (i in 1:n_simulations) {
          ...
          if (cn_is == "cat"){
            ...
            data_sim <- generate_data_frCN(n_obs, 
                                      noise_sd, 
                                      ground_truth_model, 
                                      seed)
          }
          ...
          if (cn_is == "cat"){
            ...
            metrics <- fit_models_frCN(data_sim)
          }
...
# Run simulations for fractional CN
sim_r_frCN <- run_simulations(seq(5, 15, 
                                  by = 1), 
                              noise_sd, 
                              100,
                              "frac")
...
# Reviewer 1 Section
#===================
# Alter range of variables in data generation
generate_data_r1 <- function(n, 
                          noise_sd, 
                          ground_truth_model, 
                          seed) {
  set.seed(as.numeric(seed))  # for reproducibility
...
# Alter which data generating function was used
run_simulations_r1 <- function(n_obs_vec, 
                            noise_sd_vec, 
                            n_simulations) {
    ...
    for (noise_sd in noise_sd_vec) {  
      for (ground_truth_model in 1:3) {
        for (i in 1:n_simulations) {
          ...
          # Generate the data using the function above
          data_sim <- generate_data_r1(n_obs, 
                                    noise_sd, 
                                    ground_truth_model, 
                                    seed)
...
# Parameters to simulate across
n_obs_vec_r1 <- c(10)
noise_sd_r1 <- c(0.25,1,2)
n_simulations_r1 <- 500

# Run simulations
sim_results_r1 <- run_simulations_r1(n_obs_vec_r1, 
                               noise_sd_r1, 
                               n_simulations_r1)
