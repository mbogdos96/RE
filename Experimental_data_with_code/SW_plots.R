...
# Function for plotting the square wave data, current vs potential. Requires:
...
# Function for plotting the values of extracted peaks in square wave data
...
# Separate bidentate and monodentate ligands into two plots
...
first_oxidation_potential_all_mod <- first_oxidation_potential_all %>%
  mutate(Special_label_exp = sapply(Ligand, 
                                           replace_ligand_expression_og))

# First oxidation potential
E1_plot_all <- SW_peak_plot(first_oxidation_potential_all_mod) +
  scale_x_discrete(labels = parse(
    text = first_oxidation_potential_all_mod$Special_label_exp)) +
...
first_three_oxidation_potentials_all_mod <- first_three_oxidation_potentials_all %>%
  mutate(Special_label_exp = sapply(Ligand, 
                                    replace_ligand_expression_og))

E3_plot_all <- SW_peak_plot(first_three_oxidation_potentials_all_mod) +
  scale_x_discrete(labels = parse(
    text = first_oxidation_potential_all_mod$Special_label_exp)) +
  labs(y = expression(paste(E["ox"],
                            " (V, Fc/", 
